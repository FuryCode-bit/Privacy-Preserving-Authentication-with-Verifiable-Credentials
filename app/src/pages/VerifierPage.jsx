...
import apiClient from '../api/apiClient'; // Assuming you'll use apiClient

const VerifierPage = () => {
  const [result, setResult] = useState(null);
  const [fileName, setFileName] = useState('');
  const [verifyType, setVerifyType] = useState('vc');
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      const endpoint = verifyType === 'vp' ? '/verify_presentation' : '/verify_vc';
          ...
          <CardBody className="p-4">
            <Form>
              <FormGroup tag="fieldset" className="mb-3">
                <legend className="fs-6">Verification Type</legend>
                <FormGroup check inline><Input name="verifyType" type="radio" value="vc" checked={verifyType === 'vc'} onChange={() => setVerifyType('vc')} /><Label check>Verifiable Credential (VC)</Label></FormGroup>
                <FormGroup check inline><Input name="verifyType" type="radio" value="vp" checked={verifyType === 'vp'} onChange={() => setVerifyType('vp')} /><Label check>Verifiable Presentation (VP)</Label></FormGroup>
              </FormGroup>
              <FormGroup>
